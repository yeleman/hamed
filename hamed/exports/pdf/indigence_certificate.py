...
def gen_indigence_certificate_pdf(target):
    ...
    def draw_paragraph_title(text):
        return Paragraph("""<para align=center spaceb=20 spacea=50><b><font size=12>{}</font>
                  </b></para>""".format(text), n_style)

    def draw_paragraph(data, indent=30, align="left"):
        return Paragraph(
            """<para align={align} leftIndent={indent} spaceb=2 spaceafter=1>{data}</para>""".format(
    ...
    story.append(draw_paragraph(
        """Je soussigné, {titre_maire}  le maire de la commune {name_commune}
            de {name_cercle} certifie que le nommé {name}, {naissance} à {lieu_naissance},
             {titre_enquete} {name_pere} et de {name_mere} domicilié à {adresse} <b>est indigent.</b>""".format(
            titre_maire="Madame", name_commune=commune, name_cercle=cercle, name=name, naissance=naissance,
            lieu_naissance=lieu_naissance, titre_enquete="Fille " if is_female else "Fils ",
            name_pere=name_pere, name_mere=name_mere, adresse=adresse)))

    story.append(draw_paragraph(
        "<b>NB : </b> {}".format("Ce certificat d'indigence est valable pour une duréé de six (6) mois à compter de sa date de signature.")))
    story.append(draw_paragraph(
