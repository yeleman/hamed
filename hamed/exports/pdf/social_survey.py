...
def gen_social_survey_pdf(target):
    ...
    situation_matrimoniale = instance.get("enquete/situation-matrimoniale", BLANK)
    ...
    if len(telephones) == 0:
        ...
        telephones = concat(
            [
                clean_phone_number_str(str(tel.get("enquete/telephones/numero")))
    ...
    # has_autres_revenus = get_bool(
    #     instance, 'ressources/autres-sources-revenu')
    autres_revenus = [
        (
    ...
    # total_autres_revenus = get_int(
    #     instance, 'ressources/total_autres_revenus')
    autres_revenus_f = concat(
        [
            "[{} : {}]".format(
                get_label_for("source-revenu", source_revenu), montant_revenu
            )
    ...
    autres_charges_f = concat(
        [
            "[{} : {}]".format(nature, montant_charge)
            for nature, montant_charge in autres_charges
        ],
        sep=". ",
    )
    ...
    antecedents_familiaux_details = instance.get("fantecedents/amiliaux-details", BLANK)
    antecedents_sociaux = instance.get("antecedents/sociaux")
    antecedents_sociaux_details = instance.get("antecedents/sociaux-details", BLANK)
    ...
    def draw_paragraph_title(text):
        return Paragraph(
            """<para align=center spaceb=10 spaceafter=10><b><font
            size=12>{}</font> </b></para>""".format(
                text
            ),
            style,
        )
    ...
    def draw_paragraph_sub_title_h2(text):
        return Paragraph(
            """<para align=left spaceb=8 spaceafter=8><b><font
            size=10>{}</font> </b></para>""".format(
                text
            ),
            style,
        )
    ...
    def draw_paragraph_sub_title_h3(text):
        return Paragraph(
            """<para align=left spaceb=5 spaceafter=8><b><font
            size=8>{}</font> </b></para>""".format(
                text
            ),
            style,
        )
    ...
    def draw_paragraph(label, text, indent=0):
        ...
        return Paragraph(
            """<para align=left leftIndent={indent} spaceb=2
            spaceafter=1> {label} {text}</para>""".format(
                indent=indent, label=label, text=text
            ),
            style,
        )
    ...
    headers = [
        ["MINISTÈRE DE LA SOLIDARITÉ", "", "", "REPUBLIQUE DU MALI"],
    ...
    story.append(
        draw_paragraph("Objet de l’enquête", get_label_for("objet", objet_enquete))
    )
    story.append(
        draw_paragraph("Enquête demandée par", get_label_for("demandeur", demandeur))
    )
    story.append(
        draw_paragraph_sub_title_h2("Enquêté{}".format("e" if is_female else ""))
    )
    ...
    story.append(
        draw_paragraph(
            ...
            concat(
                [
                    name,
                    "Homme" if sexe == "masculin" else "Femme",
    ...
    story.append(
        draw_paragraph("Naissance", "{} à {}".format(date_or_year, lieu_naissance))
    )
    logger.debug("Parent")
    ...
    story.append(
        draw_paragraph(
            ...
            concat(
                [
                    "{} {}".format(style_label("Père"), name_pere),
                    "{} {}".format(style_label("Mère"), name_mere),
                ]
            ),
        )
    )
    story.append(draw_paragraph("Profession", get_label_for("profession", profession)))
    story.append(draw_paragraph("Adresse", adresse))
    story.append(draw_paragraph("Téléphones", telephones))
    ...
    if epouses == []:
        ...
        for nb, epouse in enumerate(epouses):
            nom_epouse, prenoms_epouse, name_epouse = get_nom(epouse, p="epouses/e_")
            ...
            (
                type_naissance_epouse,
                annee_naissance_epouse,
                ddn_epouse,
                naissance_epouse,
                date_or_year,
            ) = get_dob(epouse, "epouses/e_", True)
            ...
            dob = "{nss_ep} à {lieu_nss_ep}".format(
                nss_ep=naissance_epouse, lieu_nss_ep=lieu_naissance_epouse
            )
                ...
                draw_paragraph(
                    ...
                    concat(
                        [
                            ...
                            "{} {}".format(style_label("Mère"), name_mere_epouse),
                        ]
                    ),
                    10,
                )
    ...
    # enfants
    story.append(
        draw_paragraph_sub_title_h3(
            "Situation des enfants (SC : "
    ...
    if enfants == []:
        ...
        for nb, enfant in enumerate(enfants):
            ...
            ) = get_lieu_naissance(enfant, "enfants/enfant_")
            (
                type_naissance_enfant,
                annee_naissance_enfant,
                ddn_enfant,
                naissance_enfant,
                date_or_year,
            ) = get_dob(enfant, "enfants/enfant_")
            # situation
            scolarise, scolarise_text = get_bool(enfant, "enfants/situation/scolarise")
            handicape, handicape_text = get_bool(enfant, "enfants/situation/handicape")
                ...
                draw_paragraph(
                    ...
                    "{nb}. {enfant}".format(
                        ...
                        enfant=concat(
                            [
                                name_enfant,
                                "M" if sexe_enfant == "masculin" else "F",
                                naissance_enfant,
                                "à {lieu}".format(lieu=lieu_naissance_enfant),
                                "SC : {}".format(
                                    get_label_for("scolarise", scolarise_text)
                                ),
                                "HD : {}".format(
                                    get_label_for("handicape", handicape_text)
                                ),
                                "AC : {}".format(
                                    get_label_for("acharge", acharge_text)
                                ),
        ...
        nb_enfants_handicap = get_int(instance, "nb_enfants_handicapes")
        ...
        story.append(
            draw_paragraph(
                ...
                concat(
                    [
                        "Nombre d'enfant : {}".format(nb_enfant),
                        "scolarisés : {}".format(nb_enfants_scolarises),
                        "handicapé : {}".format(nb_enfants_handicap),
    ...
    if autres == []:
        ...
        for nb, autre in enumerate(autres):
            nom_autre, prenoms_autre, name_autre = get_nom(autre, p="autres/autre_")
            (
                region_autre,
                cercle_autre,
                commune_autre,
            ...
            (
                type_naissance_autre,
                annee_naissance_autre,
                ddn_autre,
                naissance_autre,
                date_or_year,
            ) = get_dob(autre, "autres/autre_")
        ...
        draw_paragraph(
            ...
            concat(
                [
                    ...
                    "<u>Santé</u> : {}/mois".format(number_format(sante)),
                ],
                sep=". ",
            ),
        )
    ...
    story.append(
        draw_paragraph(
            ...
            concat(
                [
                    "<u>Type d’habitat</u> : {}".format(
                        get_label_for("type", type_habitat)
                    ),
    ...
    story.append(
        draw_paragraph(
            ...
            concat(
                [
                    get_label_for("personnels", ap)
                    for ap in antecedents_personnels.split()
                ]
            ),
        )
    )
    story.append(
        draw_paragraph("Détails antécédents personnels", antecedents_personnels_details)
    )
    story.append(
        draw_paragraph(
            "Antécédents familiaux", get_label_for("familiaux", antecedents_familiaux)
        )
    )
    story.append(
    ...
    story.append(
        draw_paragraph(
            "Antécédents sociaux", get_label_for("sociaux", antecedents_sociaux)
        )
    )
    story.append(
    ...
    story.append(
        draw_paragraph(
            ...
            concat(
                [
                    get_label_for("situation-actuelle", sa)
                    for sa in situation_actuelle.split()
                ]
            ),
        )
    )
    story.append(
        draw_paragraph("Situation actuelle details", situation_actuelle_details)
    )
    story.append(draw_paragraph("Diagnostic", get_label_for("diagnostic", diagnostic)))
    story.append(draw_paragraph("Diagnostic details", diagnostic_details))
    story.append(
        draw_paragraph(
            "L'enquêteur recommande une assistance " "sociale ?",
            get_label_for("observation", recommande_assistance_text),
        )
    )
    ...
    if sig_attachment:
        ...
        signature_img = ""
    signature = [
        ["SIGNATURE DE L’ENQUÊTEUR", "", "", "VISA DU CHEF DU SERVICE SOCIAL"],
        [signature_img, ""],
    ]
    signature_t = Table(signature, rowHeights=80, colWidths=110)
    signature_t.setStyle(
        TableStyle(
            [
                ("FONTSIZE", (0, 0), (-1, -1), 8),
